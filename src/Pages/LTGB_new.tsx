  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        const userData2nd = userData.data?.user;

        console.log("User data",userData);
        const currentRankName = getRankName(userData.currentRank || "0");
...
const useFinancialData = (
  ...
  // This function is included in both the useEffect dependency array AND returned from the hook
const fetchCurrentMonthIndex = useCallback(async (): Promise<void> => {
  if (!contract) return;

  //console.log("the nww ons is")

  try {
    const response = await rewardApi.getRABStatistics();
    console.log("the response is", response);
    const monthindex = response.data.currentMonthIndex;
    console.log("the monthindex is", response.data.currentMonthIndex);
 ...
 // Fetch financial data - separated from month index fetching
  useEffect(() => {
    const fetchFinancialData = async () => {
      ...
      try {
        ...
        const ttrab2 = rewardinfo.data.totalRABDistributed;
        setTotalRab(parseFloat(ttrab2).toFixed(3));
        //setTotalRab(ttrab2);
        setMonthlyRab(parseFloat(rewardinfo.data.arbPool).toFixed(3));
  ...
  }, [contract, isConnected, isProviderReady]);
...
const useRankDistribution = (): RankDistributionInfo => {
  ...
  useEffect(() => {
    const fetchRankDistribution = async () => {
      ...
      try {
        const response = await rewardApi.getRABStatistics();
        console.log("Fetched Rank Distribution:", response.data.eligibleCounts);

        if (response.data.eligibleCounts) {
          const formattedRankData = Object.entries(response.data.eligibleCounts).map(
            ([rank, count]) => ({
              rank: rank.replace(/_/g, " "), // Format rank names
              count: count || 0, // Ensure a number
            })
          );
  ...
  useEffect(() => {
    const fetchTimerData = async () => {
        ...
        try {
              ...
              const response = await rewardApi.getRewardStats();

              const startTimestamp = response.data.rabStartTimestamp; // "2025-03-08T08:57:58.665Z"
              console.log("Fetched Start Timestamp:", startTimestamp);
              
              // Convert ISO string to UNIX timestamp (in seconds)
              const start = Math.floor(new Date(startTimestamp).getTime() / 1000);
              console.log("Start Timestamp (UNIX format):", start);
              
              const currentTime = Math.floor(Date.now() / 1000); // Current UNIX timestamp (seconds)
              console.log("Current Time (UNIX format):", currentTime);
              ...
              console.log("Next Cycle Start (UNIX format):", nextCycleStart);

            console.log("Computed Start Time:", new Date(start * 1000).toLocaleString());
            console.log("Next Cycle Start:", new Date(nextCycleStart * 1000).toLocaleString());
